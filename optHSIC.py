...
def opt_hsic(x,z,d,seed,num_permutations=1999):
    n=np.shape(x)[0]
    ...
    for i in range(n):
        ...
        if d[i]==1:
            #if it's an observed event observed, compute the coupling
    ...
    #the new data is now ready, compute the matrices for HSIC
